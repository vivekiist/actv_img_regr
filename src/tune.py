...
def select_uncertain_samples(args, g_model, train_loader):
	g_model.eval()
...
def active_learning(args, epoch, run_logger, g_model, train_loader, train_dataset, kwargs):
...
def save_checkpoint(epoch, args, g_model, g_optimizer, g_losses=None, batch_train_losses=None, batch_test_losses=None, batch_ssim=None, batch_psnr=None, batch_lpips=None, train_losses=None, test_losses=None, test_ssim=None, test_psnr=None, test_lpips=None, run_logger = None, main_logger = None, d_model=None, d_optimizer=None, d_losses=None):
	...
	torch.save(checkpoint, chkname)
	torch.save(g_model.state_dict(), mname)
	main_logger.info('Model & Checkpoint saved at epoch %s', epoch)

def last_epoch_actions(args, main_logger, g_model, epoch, train_losses, test_losses, test_ssim, test_psnr, test_lpips, test_loader, device):
...
def train_model():	
	...
	for epoch in tqdm(range(args.start_epoch, args.epochs)):
		...
		if (not args.no_active_learning) and (len(train_dataset) < args.sampling_budget):
			active_learning(args, epoch, run_logger, g_model, train_loader, train_dataset, kwargs)
		...
		if (((epoch+1) % args.check_every == 0) or (epoch == args.epochs-1)) :
			if args.use_gan_loss:
				save_checkpoint(epoch, args, g_model, g_optimizer, g_losses, batch_train_losses, batch_test_losses, batch_ssim, batch_psnr, batch_lpips, train_losses, test_losses, test_ssim, test_psnr, test_lpips, run_logger, main_logger, d_model, d_optimizer, d_losses)
			else:
				save_checkpoint(epoch, args, g_model, g_optimizer, g_losses, batch_train_losses, batch_test_losses, batch_ssim, batch_psnr, batch_lpips, train_losses, test_losses, test_ssim, test_psnr, test_lpips, run_logger, main_logger, d_model=None, d_optimizer=None, d_losses=None)

		if (epoch == args.epochs-1):
			last_epoch_actions(args, main_logger, g_model, epoch, train_losses, test_losses, test_ssim, test_psnr, test_lpips, test_loader, device)
